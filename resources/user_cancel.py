...
class cancelBooking(Resource):
    ...
    def delete(self):
        ...
        #result={}
        try:
            log1=query(f"""Select * from bookingHistory1 where user_id='{data["id"]}' and  date_format(day,"%Y-%m-%d")=date_format(curdate(),"%Y-%m-%d") and status<>1""",return_json=False)
            
            if(len(log1)==0):
                return {"message": "Can't Cancel your booking request"}, 500
            else:
                query(f"""DELETE from booking where user_id='{data["id"]}' and  date_format(day,"%Y-%m-%d")=date_format(curdate(),"%Y-%m-%d") and status<>1""")
                #result["1"]=log1
